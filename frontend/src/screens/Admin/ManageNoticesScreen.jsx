...
export default function ManageNoticesScreen({ navigation }) {
  ...
  const handleAddGDriveUrl = () => {
    ...
    if (!convertedUrls) {
      Alert.alert(
        "Error",
        "Invalid Google Drive URL. Please check and try again."
      );
      return;
    }
  ...
  const removeAttachment = async (index) => {
    try {
      ...
      if (editMode && selectedNotice && attachment.source === "server-upload") {
        await api.delete(
          `/files/notice/${selectedNotice._id}/attachment/${attachment._id}`
        );
      }
  ...
  const toggleStatus = async (notice) => {
    try {
      ...
      if (data.success) {
        setNotices(
          notices.map((n) => (n._id === notice._id ? data.notice : n))
        );
                ...
                <View className="flex-row flex-wrap gap-2 mb-4">
                  {priorities.map((p) => (
                    <TouchableOpacity
                      ...
                      style={{
                        backgroundColor:
                          priority === p ? getPriorityColor(p) : undefined,
                      }}
                ...
                <View className="mb-4">
                  ...
                  {uploadingFile && (
                    <View className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 mb-3 flex-row items-center">
                      ...
                      <Text className="text-blue-600 dark:text-blue-400 ml-3 font-semibold">
                        Uploading file...
                      </Text>
                    </View>
                  )}
